...
config.outlier_percentage = -1
config.epoch_model_used = -1
config.loss = "l1" #default loss
config.jitter = 0 #is jitter
config.norm = 0
config.GAN = 0
config.load_path_generator = ''
config.load_path_discriminator = ''
config.head_discriminator = "None"
config.freeze_gen = 0
config.architecture = "U-Net"
config.noise_type = 'gaussian'
config.sample_Dl_patches = 0.05 #patch_radius
...
# config.aug_rot= [0,0,0]
# config.aug_sym= [0,0,0]
...
config.attention = edict()
config.attention.type = 'Non-local'
config.pointwisemlp.feature_type = 'dp_df'
config.pointwisemlp.num_mlps = 1
config.pointwisemlp.reduction = 'max'
