...
def get_scheduler(optimizer, n_iter_per_epoch, args):
    ...
    if "cosine" in args.lr_scheduler:
        ...
        lr_decay_epochs = [args.lr_decay_steps * i for i in range(1, int(args.epochs // args.lr_decay_steps))]
        scheduler = MultiStepLR(
            optimizer=optimizer,
            gamma=args.lr_decay_rate,
            milestones=[(m - args.warmup_epoch) * n_iter_per_epoch for m in lr_decay_epochs])
    elif "step_PCN" == args.lr_scheduler:
        print("special scheduler for PCN")
        scheduler = MultiStepLR(
            optimizer=optimizer,
            gamma=args.lr_decay_rate,
            milestones=[])
