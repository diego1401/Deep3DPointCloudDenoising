...
class MaskedUpsample(nn.Module):
    ...
    def forward(self, up_xyz, xyz, up_mask, mask, features):
        ...
        if self.mode == 'nearest':
            #dists = torch.linalg.norm(grouped_xyz.squeeze(-1)-up_xyz.transpose(-1,-2),dim=1)
            up_feature = neighborhood_features[..., 0].contiguous()#/(1.+dists.unsqueeze(1))
        elif self.mode == 'max':
            ...
            # Use radial basis function kernel for weighted sum of features (normalized by nsample and sigma:=radius)
            # Ref: https://en.wikipedia.org/wiki/Radial_basis_function_kernel
            rbf = torch.exp(-1 * grouped_xyz.pow(2).sum(1,keepdim=False) / (1**2) / 2) # (B, npoint, nsample)
            up_feature = torch.sum(neighborhood_features * rbf.unsqueeze(1), -1, keepdim=True) / float(self.nsample) # (B, mlp[-1], npoint, 1)
        else:
            raise NotImplementedError(f"mode:{self.mode} not supported in MaskedUpsample")
        up_feature = up_feature.squeeze(-1)  # (B, mlp[-1], npoint)
        return up_feature
