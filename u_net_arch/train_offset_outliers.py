...
from models import build_complete_denoising
...
def get_loader(config):
    ...
    train_dataset = OffsetDataset(input_features=config.features, katz_params=config.katz_params, 
                                ...
                                outlier_proportion=config.outlier_percentage, DEBUG=config.DEBUG,fourier_features=config.fourier_features)
...
def aux_compute_loss(config,criterion,to_unpack):
    pred, points_offsets,points_labels, mask, points = to_unpack
    criterion_offset,criterion_outlier = criterion

    oi = nn.Sigmoid()(pred[...,-1])
    di = nn.Tanh()(pred[...,:-1])

    if config.loss == "double_weight":
        loss_outliers = criterion_outlier(oi,points_labels,points_offsets,mask)
    elif config.loss in ["L1_classification","Weighted_L1_classification"]:
        loss_outliers = criterion_outlier(oi,points_labels)
    else:
        raise ValueError(f"Loss {config.loss} not implemented.")

    if config.loss =="L1_classification":
        loss_offset = criterion_offset(di, points_offsets, mask)
    elif config.loss == "double_weight":
        loss_offset = criterion_offset(di, points_offsets, mask)
    elif config.loss == "Weighted_L1_classification":
        threshold = torch.tensor([0.5]).cuda(non_blocking=True).float()
        predicted_outlier_labels = torch.heaviside(oi-0.5,threshold) # if oi >= 0.5 -> 1 else 0
        new_mask = torch.logical_or(mask,predicted_outlier_labels).bool()
        loss_offset = criterion_offset(di, points_offsets, new_mask)
    else:
        raise ValueError(f"Loss {config.loss} not implemented.")

    print(f"Loss offset: {loss_offset}\nLoss outliers: {loss_outliers}\n")
    return loss_offset + loss_outliers*config.in_radius


def get_loss(config,criterion,model,to_unpack):
    points, mask, features, points_labels, points_offsets, current_cloud_index, input_inds = to_unpack
    # forward
    ...
    #loss
    loss = aux_compute_loss(config,criterion,(pred, points_offsets,points_labels, mask, points))
...
def main(config):
    ...
    model,(criterion_offset,criterion_outlier) = build_complete_denoising(config)
    model.cuda()
    criterion_offset.cuda()
    criterion_outlier.cuda()
    criterion = (criterion_offset,criterion_outlier)
...
def train(epoch, train_loader, model, criterion, optimizer, scheduler, config):
    ...
    for idx,unpack in enumerate(train_loader):
        optimizer.zero_grad()
        data_time.update(time.time() - end)
        bsz = unpack[0].size(0) #unpack[0] == points
        
        loss = get_loss(config,criterion,model,unpack)
...
def validate(epoch, test_loader, model, criterion, config):
    ...
    with torch.no_grad():
        ...
        for idx, unpack in enumerate(test_loader):
            bsz = unpack[0].size(0) #unpack[0] == points
            loss = get_loss(config,criterion,model,unpack)
