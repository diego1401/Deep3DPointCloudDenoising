...
class Attention(nn.Module):
    def __init__(self, in_channels, out_channels, radius, nsample, config):
        ...
        self.attention_type = config.attention.type
        ...
        if self.attention_type=="Point-transformer":
            self.sa = None
        else:
            self.sa = PointWiseMLP(in_channels, out_channels, radius, nsample, config)

        if self.attention_type=="Non-local":
            self.attention_module = NonLocalModule(self.in_channels)
        elif self.attention_type=="Criss-cross":
            self.attention_module = CrissCrossAttention(self.in_channels)
        elif self.attention_type=="SE":
            self.attention_module = SE(self.in_channels)
        elif self.attention_type=="CBAM":
            self.attention_module = CBAMAttention(self.in_channels)
        elif self.attention_type=="Dual-attention":
            self.attention_module = DualAttention(self.in_channels)
        elif self.attention_type=="A-SCN":
            self.attention_module = ShapeContext(self.in_channels)
        elif self.attention_type=="Point-attention":
            self.attention_module = PointAttentionNetwork(self.in_channels)
        elif self.attention_type=="CAA":
            self.attention_module = CAA_Module(self.in_channels,self.nsample)
        elif self.attention_type=="Offset-attention":
            self.attention_module = OffsetAttention(self.in_channels)
        elif self.attention_type=="Point-transformer":
            self.attention_module = Point_Transformer(self.in_channels, radius, nsample)
    def forward(self, query_xyz, support_xyz, query_mask, support_mask, support_features):
        ...
        if self.attention_type=="Point-transformer":
            out_features = self.attention_module(query_xyz, support_xyz, query_mask, support_mask, support_features)
        else:
            out_features = self.sa(query_xyz, support_xyz, query_mask, support_mask, support_features)
            out_features = self.attention_module(out_features)
...
class PointWiseMLP(nn.Module):
    ...
    def forward(self, query_xyz, support_xyz, query_mask, support_mask, support_features):
        ...
        if self.feature_type == 'dp_fi_df':
            ...
            center_features = torch.unsqueeze(neighborhood_features[..., 0], -1).repeat([1, 1, 1, self.nsample])
            relative_features = neighborhood_features - center_features
            local_input_features = torch.cat([relative_position, relative_features], 1)
            aggregation_features = self.mlps(local_input_features)
        else:
...
class LocalAggregation(nn.Module):
    def __init__(self, in_channels, out_channels, radius, nsample, config):
        ...
        elif config.local_aggregation_type == 'attention':
            print("Using attention of type {}".format(config.attention.type))
            self.local_aggregation_operator = Attention(in_channels, out_channels, radius, nsample, config)
        else:
